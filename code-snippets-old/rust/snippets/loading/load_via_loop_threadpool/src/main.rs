...
fn main() -> SzResult<()> {
    ...
    println!("==================================");
    println!("Loading {} records using {} OS threads for maximum performance", TOTAL_RECORDS, WORKER_COUNT);
    ...
    if loaded > 0 {
        println!("Average loading rate: {:.2} records/second",
                loaded as f64 / total_time.as_secs_f64());
        println!("Per-thread rate: {:.2} records/second/thread",
...
fn process_load_task(engine: &Box<dyn SzEngine>, task: &LoadTask) -> SzResult<()> {
    engine.add_record(&task.data_source, &task.record_id, &task.record_data, Some(SzFlags::ADD_RECORD_DEFAULT))?;
    Ok(())
}