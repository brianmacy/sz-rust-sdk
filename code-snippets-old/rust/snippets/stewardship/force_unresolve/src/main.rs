...
fn main() -> SzResult<()> {
    ...
    let primary_entity_id = if entity_id1 == entity_id2 && entity_id2 == entity_id3 {
        println!("\n✅ All records resolved to the same entity ({})", entity_id1);
        entity_id1
    } else if entity_id1 == entity_id2 {
        println!("\n✅ Records 1 and 2 resolved to the same entity ({})", entity_id1);
        entity_id1
    } else if entity_id1 == entity_id3 {
        println!("\n✅ Records 1 and 3 resolved to the same entity ({})", entity_id1);
        entity_id1
    } else if entity_id2 == entity_id3 {
        println!("\n✅ Records 2 and 3 resolved to the same entity ({})", entity_id2);
        entity_id2
    } else {
    ...
    let reload_info = engine.add_record("TEST", "FORCE_UNRESOLVE_2", modified_record2, Some(load_flags))?;
    ...
    println!("Original entity {} (should have fewer records now)", primary_entity_id);
    println!("New/separate entity {} for the unresolved record", new_entity_id2);
    ...
    println!("Updated original entity: {}", get_entity_summary(&updated_entity1)?);
    ...
    println!("Final original entity: {}", get_entity_summary(&final_entity1)?);
    println!("Final separate entity: {}", get_entity_summary(&final_entity2)?);
...
fn extract_entity_id(info_json: &str, record_description: &str) -> SzResult<i64> {
    let info: Value = serde_json::from_str(info_json)
        .map_err(|e| SzError::bad_input(&format!("Failed to parse info for {}: {}", record_description, e)))?;

    info["RESOLVED_ENTITY"]["ENTITY_ID"]
        .as_i64()
        .or_else(|| info["AFFECTED_ENTITIES"].as_array()?.first()?.get("ENTITY_ID")?.as_i64())
        .ok_or_else(|| SzError::bad_input(&format!("Could not extract entity ID from {}", record_description)))
}
...
fn get_entity_summary(entity_json: &str) -> SzResult<String> {
    ...
    let entity_id = entity["RESOLVED_ENTITY"]["ENTITY_ID"]
        .as_i64()
        .unwrap_or(0);
    ...
    Ok(format!("Entity {} with {} record(s): {:?}", entity_id, records, record_ids))
}
