...
fn main() -> SzResult<()> {
    ...
    println!("Loading {} test records using loop pattern...", NUM_TEST_RECORDS);
    ...
    for record_num in 1..=NUM_TEST_RECORDS {
        ...
        let line = serde_json::to_string(&test_record)
            .map_err(|e| SzError::unknown(&format!("Failed to serialize test record {}: {}", record_num, e)))?;
