...
fn load_file(engine: &Box<dyn SzEngine>, file_path: &str) -> SzResult<()> {
    ...
    let retry_file_path = format!("{}{}{}", RETRY_PREFIX,
        std::path::Path::new(file_path).file_stem().unwrap().to_string_lossy(), RETRY_SUFFIX);
    let mut retry_writer = File::create(&retry_file_path)
        .map_err(|e| SzError::unknown(&format!("Failed to create retry file {}: {}", retry_file_path, e)))?;
        ...
        match process_record_with_redo(&engine, &line, line_number) {
            Ok(()) => {
                ...
                if SUCCESS_COUNT.load(Ordering::Relaxed) % 100 == 0 {
                    println!("Processed {} records successfully", SUCCESS_COUNT.load(Ordering::Relaxed));
                }
            }
            Err(e) => {
                ERROR_COUNT.fetch_add(1, Ordering::Relaxed);
...
fn process_record_with_redo(engine: &Box<dyn SzEngine>, json_line: &str, line_number: usize) -> SzResult<()> {
    ...
    let data_source = record.get(DATA_SOURCE_KEY)
        .and_then(|v| v.as_str())
        .ok_or_else(|| SzError::bad_input(&format!("Missing {} on line {}", DATA_SOURCE_KEY, line_number)))?;

    let record_id = record.get(RECORD_ID_KEY)
        .and_then(|v| v.as_str())
        .ok_or_else(|| SzError::bad_input(&format!("Missing {} on line {}", RECORD_ID_KEY, line_number)))?;

    // Add the record to the repository with no flags
    let _response = engine.add_record(
        data_source,
        record_id,
        json_line,
        None,
...
fn process_redo_records(engine: &Box<dyn SzEngine>) -> SzResult<()> {
    ...
    println!("Redo processing completed after {} iterations", iteration - 1);
    Ok(())
}
...
fn process_single_redo_record(_engine: &Box<dyn SzEngine>, redo_record: &str) -> SzResult<()> {
    ...
    if REDONE_COUNT.load(Ordering::Relaxed) % 10 == 0 {
        println!("Processed {} redo records", REDONE_COUNT.load(Ordering::Relaxed));
    }
