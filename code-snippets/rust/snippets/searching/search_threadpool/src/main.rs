...
fn main() -> SzResult<()> {
    ...
    println!("===========================");
    println!("Executing {} search queries using {} OS threads for maximum performance", SEARCH_QUERIES, WORKER_COUNT);
    ...
    println!("\n2. Starting {} worker threads for concurrent search operations...", WORKER_COUNT);
            ...
            while running.load(Ordering::Relaxed) {
                ...
                match task {
                    Ok(search_task) => {
                        ...
                        match result {
                            Ok(search_result) => {
                                let current = SEARCHES_COMPLETED.fetch_add(1, Ordering::Relaxed) + 1;
                                MATCHES_FOUND.fetch_add(search_result.match_count, Ordering::Relaxed);

                                if current % STATS_REPORT_INTERVAL == 0 {
                                    println!("Worker {}: Completed {} searches", worker_id, current);
                                }

                                if let Err(_) = result_sender.send(search_result) {
    ...
    if searches > 0 {
        println!("Average search rate: {:.2} searches/second",
                searches as f64 / total_time.as_secs_f64());
        println!("Per-thread rate: {:.2} searches/second/thread",
...
fn load_test_data(engine: &Box<dyn SzEngine>) -> SzResult<()> {
    let test_records = vec![
        (r#"{"RECORD_ID": "SEARCH_TEST_001", "DATA_SOURCE": "TEST", "NAME_FIRST": "Alice", "NAME_LAST": "Johnson", "EMAIL_ADDRESS": "alice.johnson@email.com", "PHONE_NUMBER": "555-0001"}"#, "SEARCH_TEST_001"),
        (r#"{"RECORD_ID": "SEARCH_TEST_002", "DATA_SOURCE": "TEST", "NAME_FIRST": "Bob", "NAME_LAST": "Smith", "EMAIL_ADDRESS": "bob.smith@email.com", "PHONE_NUMBER": "555-0002"}"#, "SEARCH_TEST_002"),
        (r#"{"RECORD_ID": "SEARCH_TEST_003", "DATA_SOURCE": "TEST", "NAME_FIRST": "Carol", "NAME_LAST": "Williams", "EMAIL_ADDRESS": "carol.williams@email.com", "PHONE_NUMBER": "555-0003"}"#, "SEARCH_TEST_003"),
        (r#"{"RECORD_ID": "SEARCH_TEST_004", "DATA_SOURCE": "TEST", "NAME_FIRST": "David", "NAME_LAST": "Brown", "EMAIL_ADDRESS": "david.brown@email.com", "PHONE_NUMBER": "555-0004"}"#, "SEARCH_TEST_004"),
        (r#"{"RECORD_ID": "SEARCH_TEST_005", "DATA_SOURCE": "TEST", "NAME_FIRST": "Emma", "NAME_LAST": "Davis", "EMAIL_ADDRESS": "emma.davis@email.com", "PHONE_NUMBER": "555-0005"}"#, "SEARCH_TEST_005"),
    ];

    for (record_data, record_id) in test_records {
        engine.add_record("TEST", record_id, record_data, Some(SzFlags::ADD_RECORD_DEFAULT))?;
    }
...
fn generate_search_tasks() -> Vec<SearchTask> {
    let search_patterns = vec![
        ...
        ("Email Search", r#"{"EMAIL_ADDRESS": "alice.johnson@email.com"}"#),
        ("Phone Search", r#"{"PHONE_NUMBER": "555-0002"}"#),
        ("Combined Search", r#"{"NAME_FIRST": "Bob", "NAME_LAST": "Smith"}"#),
        ("Partial Name", r#"{"NAME_FIRST": "Car"}"#),
        ("Domain Search", r#"{"EMAIL_ADDRESS": "@email.com"}"#),
        ("Phone Pattern", r#"{"PHONE_NUMBER": "555-000"}"#),
        ("Full Name", r#"{"NAME_FIRST": "David", "NAME_LAST": "Brown"}"#),
...
fn analyze_search_results(results: &[SearchResult], total_time: Duration) {
    ...
    let avg_search_time: f64 = results.iter().map(|r| r.search_time_ms as f64).sum::<f64>() / results.len() as f64;

    let fastest = results.iter().min_by_key(|r| r.search_time_ms).unwrap();
    let slowest = results.iter().max_by_key(|r| r.search_time_ms).unwrap();
    ...
    println!("  Fastest search: {} ({:.2}ms)", fastest.search_name, fastest.search_time_ms);
    println!("  Slowest search: {} ({:.2}ms)", slowest.search_name, slowest.search_time_ms);
    println!("  Most productive: {} ({} matches)", most_matches.search_name, most_matches.match_count);

    if total_time.as_secs_f64() > 0.0 {
        println!("  Concurrent efficiency: {:.2}x faster than sequential",
