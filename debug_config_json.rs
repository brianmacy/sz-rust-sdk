...
fn main() -> SzResult<()> {
    ...
    let result = unsafe { sz_rust_sdk::ffi::bindings::SzConfigMgr_getConfig_helper(config_id) };
    ...
    let config_json = unsafe { sz_rust_sdk::ffi::helpers::c_str_to_string(result.response) }?;

    println!("=== JSON from SzConfigMgr_getConfig_helper ===");
    println!("Length: {}", config_json.len());
    println!("First 500 chars: {}", &config_json[..config_json.len().min(500)]);
    println!("Last 200 chars: {}", &config_json[config_json.len().saturating_sub(200)..]);
    ...
    // Create empty config
    let create_result = unsafe { sz_rust_sdk::ffi::bindings::SzConfig_create_helper() };
    ...
    let config_def_c = sz_rust_sdk::ffi::helpers::str_to_c_string(&config_json)?;
    let load_result = unsafe {
        sz_rust_sdk::ffi::bindings::SzConfig_load_helper(
    ...
    if load_result == 0 {
        ...
        println!("❌ SzConfig_load_helper failed with code: {}", load_result);

        // Get the actual error message
        let mut error_buffer = vec![0u8; 1024];
        let error_len = unsafe {
            sz_rust_sdk::ffi::bindings::SzConfig_getLastException(
    ...
    // Clean up
    unsafe {
        sz_rust_sdk::ffi::bindings::SzConfig_close_helper(create_result.response);
    }
