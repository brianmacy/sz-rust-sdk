...
fn main() -> SzResult<()> {
    ...
    println!("✅ Set configuration {current_config_id} as default");
    ...
    for (data_source, description) in &data_sources_to_test {
        ...
        let test_record = format!(
            r#"{{
            "RECORD_ID": "TEST_{data_source}",
            "DATA_SOURCE": "{data_source}",
            "NAME_FIRST": "Test",
            "NAME_LAST": "Person{data_source}",
            "NAME_FULL": "Test Person{data_source}"
        }}"#
        );
        ...
        match engine.add_record(
            ...
            Err(e) => {
                println!("  ❌ Data source {data_source} failed: {e}");
    ...
    if !failed_data_sources.is_empty() {
        ...
        return Err(SzError::configuration(format!(
            "Data source registration failed for: {failed_data_sources:?}"
        )));
    }
    ...
    match final_config_mgr.get_config_registry() {
        Ok(registry) => {
            ...
            println!("Found {ds_count} data source entries in registry");
        }
        Err(e) => {
